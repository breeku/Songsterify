...
const trackReducer = (state = null, action) => {
    if (action.type === "GET_TRACKS") {
        return {...state, [action.id]: {...action.data, differentArtists: action.differentArtists}} 
...
export const getTracks = obj => {
    return async dispatch => {
        ...
        for (const x of tracks.tracks.items) {
            ...
            if (!differentArtists.find(x => x === name)) differentArtists.push(name)
        }
        dispatch({ type: "GET_TRACKS", data: tracks, id: obj.id, differentArtists: differentArtists.length })
    }
}
